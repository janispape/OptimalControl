...
We want to find a control $\alpha$ in some set$\mathcal{A} \subset \\{ \alpha:[0,\infty) \rightarrow \mathbb{R}^m | \ \alpha \text{ is measurable}
\\}$ (often, $\mathcal{A}$ is just a restriction of the image of the $\alpha$),
...
($x_\alpha$ is the solution of the ODE, $T$ is allowed to be infinite). Details can be found here:
- [Evans — An Introduction to Mathematical Optimal Control Theory](https://math.berkeley.edu/~evans/control.course.pdf)
    - this script introduces the analytical theory, it also contains the examples implemented in this repo
- [Grüne & Pannek — Numerical Methods for Nonlinear Optimal Control Problems](https://epub.uni-bayreuth.de/id/eprint/2001/1/Gruene_num_meth_nonlin_optimal_control_2013.pdf)
    - this article discusses the numerical methods used

### Rocket Car

<img align="right" src="images/rocket_car_img.png" width="220" alt="Rocket car" style="margin-left: 15px; margin-bottom: 10px; border-radius: 10px;">
...
<img align="left" src="images/moon_lander_img.png" width="120" alt="Moon lander" style="margin-right: 15px; margin-bottom: 10px; border-radius: 10px;">

In this example the height and velocity of a spacecraft close to the moon is given. Due to gravity it
accelerates towards the surface of the astronomical body. Intuitively it is clear that we have to fire the 
rockets at the bottom of the carrier at some point to guarantee a smooth landing. We also want to minimise fuel
use. Here, we have to be careful (this makes it a nice example for optimal control problems): Using up fuel 
decreases the mass of the rocket which obviously has an impact on the ODE governing its position and velocity. 
The details of the problem are extensively discussed in Evans script on p.55. We first discretize the problem
and thereafter use classic constrained nonlinear optimization techniques to solve it, namely sequential
quadratic programming (SQP). The example is implemented in ```moon_lander.py```.
